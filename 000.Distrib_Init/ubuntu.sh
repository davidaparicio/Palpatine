...
do_setup_pkg_base () {
  ...
  if ! dpkg -s apt-transport-https | grep installed | grep ok
  then
    do_base_setup=true
  fi
    python-software-properties
  if ! dpkg -s software-properties-common | grep installed | grep ok
  then
    do_base_setup=true
  fi
  if ! dpkg -s python-software-properties | grep installed | grep ok
  then
    do_base_setup=true
  fi
  if [[ ${do_base_setup} ]]
